...
import { getHighResGooglePhoto, getUserInitials } from '../lib/avatar-utils';
...
export default function AccountSettings() {
  ...
  const rawPhotoUrl = profile?.photoURL || profile?.photo || user?.photoURL;
  const avatarUrl = getHighResGooglePhoto(rawPhotoUrl);
  const [imageError, setImageError] = useState(false);
  const userInitials = getUserInitials(profile?.fullName, user?.email);
          ...
          <div className="bg-white rounded-[2rem] border border-secondary p-8 space-y-6 shadow-xl relative overflow-hidden">
            ...
            <div className="text-center space-y-4">
              <div className="relative w-24 h-24 mx-auto">
                <div className="w-24 h-24 rounded-full p-1 bg-gradient-to-tr from-[#D4AF37] via-[#FFFDF0] to-[#95731C] shadow-lg shadow-[#D4AF37]/20 flex items-center justify-center overflow-hidden">
                  {avatarUrl && !imageError ? (
                    ...
                    <div className="w-full h-full bg-primary-dark text-white rounded-full flex items-center justify-center text-2xl font-bold tracking-wider uppercase shadow-inner">
                      {userInitials}
                    </div>
                  )}
                </div>
                {isGoogleUser && (
                  <div 
                    className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-white border-2 border-[#D4AF37] shadow-md flex items-center justify-center"
